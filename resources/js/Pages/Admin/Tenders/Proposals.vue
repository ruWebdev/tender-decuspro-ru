                    ...
                    <tbody>
                        <tr v-if="props.proposals.length === 0">
                            <td colspan="4" class="text-center text-muted">
                                {{ t('proposals.customer_no_proposals') }}
                            </td>
                        </tr>
                        <tr v-for="proposal in props.proposals" :key="proposal.id">
                            ...
                            <td>
                                <span class="badge bg-blue text-light">{{ proposal.items_count ?? 0 }}</span>
                            </td>
                            <td>
                                <div class="btn-list flex-nowrap">
                                    <Link :href="route('admin.proposals.show', { proposal: proposal.id })"
                                        class="btn btn-sm btn-ghost-primary">
                                    {{ t('proposals.customer_action_open') }}
                                    </Link>
                                    <Link v-if="!props.tender?.is_finished"
                                        :href="route('admin.tenders.finish', { tender: props.tender.id })"
